...
public class VMSnapshot extends BaseCommand {
	...
	boolean getTaskResultAfterDone(ManagedObjectReference task)
		...
		Object[] result = vcService.getWaitForValues().wait(task, new String[] { "info.state",
	...
	boolean createSnapshot(ManagedObjectReference vmMor)
		...
		ManagedObjectReference taskMor = vcService.getVimPort().createSnapshotTask(vmMor,
	...
	boolean listSnapshot(ManagedObjectReference vmMor)
			throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
		VirtualMachineSnapshotInfo snapInfo = (VirtualMachineSnapshotInfo) vcService.getGetMOREFs()
	...
	boolean revertSnapshot(ManagedObjectReference vmMor)
		...
		if (snapmor != null) {
			ManagedObjectReference taskMor = vcService.getVimPort().revertToSnapshotTask(
					snapmor, null, true);
			if (getTaskResultAfterDone(taskMor)) {
	...
	boolean removeAllSnapshot(ManagedObjectReference vmMor)
			...
			InvalidPropertyFaultMsg, InvalidCollectorVersionFaultMsg {
		ManagedObjectReference taskMor = vcService.getVimPort().removeAllSnapshotsTask(vmMor,
				true);
		if (taskMor != null) {
			String[] opts = new String[] { "info.state", "info.error",
					"info.progress" };
			String[] opt = new String[] { "state" };
			Object[] results = vcService.getWaitForValues().wait(taskMor, opts, opt,
					new Object[][] { new Object[] { TaskInfoState.SUCCESS,
							TaskInfoState.ERROR } });
	...
	boolean removeSnapshot(ManagedObjectReference vmMor)
		...
		ManagedObjectReference snapmor = getSnapshotReference(vmMor,
				virtualMachineName, snapshotname);
		if (snapmor != null) {
			ManagedObjectReference taskMor = vcService.getVimPort().removeSnapshotTask(
					snapmor, flag, true);
			if (taskMor != null) {
				String[] opts = new String[] { "info.state", "info.error",
						"info.progress" };
	...
	public void createSnapshot() throws InvalidPropertyFaultMsg, TaskInProgressFaultMsg,
		...
		ManagedObjectReference propCol = vcService.getConnection().getServiceContent()
				.getPropertyCollector();
		ManagedObjectReference vmRef = vcService.getGetMOREFs().vmByVMname(virtualMachineName,
				propCol);
	...
	public boolean snapshotExists() throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
		boolean valid = false;
		valid = verifyInputArguments();
		if (!valid) {
