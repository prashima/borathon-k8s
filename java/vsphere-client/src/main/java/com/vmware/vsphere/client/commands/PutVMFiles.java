...
import com.vmware.vsphere.client.connection.KeepAlive;
 ...
 *
 */
public class PutVMFiles extends BaseCommand {
    ...
    public void setLocalPath(final String localPath) {
        // the program will look in here for the file.
        ...
        try {
            if (datacenter != null && datacenter.length() != 0
                ...
                ManagedObjectReference dcmor =
                        vcService.getGetMOREFs().inContainerByType(vcService.getServiceContent().getRootFolder(),
                                "Datacenter").get(datacenter);
                if (dcmor != null) {
                    ManagedObjectReference ds =
                            vcService.getGetMOREFs().inContainerByType(dcmor, "Datastore").get(datastore);
                    if (ds == null) {
    ...
    public void putFile() {
    	 ...
    	 putVMFiles(remotePath, localFile);
    }

    @SuppressWarnings("unchecked")
    void putVMFiles(final String remoteFilePath, final File localFile) {
        final String url = vcService.getConnection().getUrl();
        ...
        // Maintain session
        final List<String> cookies = (List<String>) vcService.getHeaders().get("Set-cookie");
        ...
        try {
            ...
            Thread keepAlive = KeepAlive.keepAlive(vcService.getVimPort(), vcService.getServiceInstanceReference());
            keepAlive.start();
    ...
    boolean registerVirtualMachine() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg, OutOfBoundsFaultMsg, DuplicateNameFaultMsg, NotFoundFaultMsg, VmConfigFaultFaultMsg, InsufficientResourcesFaultFaultMsg, AlreadyExistsFaultMsg, InvalidDatastoreFaultMsg, FileFaultFaultMsg, InvalidStateFaultMsg, InvalidNameFaultMsg, InvalidCollectorVersionFaultMsg {
        ...
        final List<DatastoreHostMount> hostmounts =
                ((ArrayOfDatastoreHostMount) vcService.getGetMOREFs().entityProps(dsmor,
        ...
        final ManagedObjectReference taskmor =
                vcService.getVimPort().registerVMTask(vmFolderMor, vmxPath, vmName, false,
    ...
    void reconfigVirtualMachine() {
        try {
            ...
            ManagedObjectReference taskmor =
                    vcService.getVimPort().reconfigVMTask(registeredVMRef, vmConfigSpec);
    ...
    void putVMFiles() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {
        ...
        if (vcService.getGetMOREFs().inContainerByType(vcService.getServiceContent().getRootFolder(), "VirtualMachine").containsKey(vmName)) {
